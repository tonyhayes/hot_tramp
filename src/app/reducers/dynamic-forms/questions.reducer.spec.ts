...
describe('questions Reducer', () => {
  	...
  	it('uses an initial state when none is given', () => {
		let result = questionsReducer(undefined, {type: 'SOME ACTION'});
		expect(result.length).toBe(0);
  	});
  	it('return payload when LOAD_QUESTIONS_SUCCESS is given', () => {
		let result = questionsReducer(undefined, actions.loadQuestionsSuccess(QuestionState));
		expect(result[0].label).toBe('Test');
  	});
  	it('return payload when SAVE_QUESTIONS_SUCCESS is given', () => {
		let result = questionsReducer(undefined, actions.saveQuestionsSuccess(QuestionState));
		expect(result[0].label).toBe('Test');
  	});
