import reducer, * as fromUserAdministration from './user-administration.reducer';
...
describe('questions Reducer', () => {
  	...
  	it('return payload when LOAD_USERS_SUCCESS is given', () => {
		let result = reducer(undefined, actions.loadUsersSuccess(UserState));
		expect(result[0].firstName).toBe('Test');
  	});
  	it('return payload when SAVE_USERS_SUCCESS is given', () => {
		let result = reducer(undefined, actions.saveUsersSuccess(UserState));
		expect(result[0].firstName).toBe('Test');
  	});
